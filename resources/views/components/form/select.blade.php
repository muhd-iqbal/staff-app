...
<div class="col-span-6 sm:col-span-{{ $span }}">
    ...
    <select id="{{ $name }}" name="{{ $name }}" autocomplete="{{ $name }}"
        class="mt-1 block w-full py-2 px-3 border border-purple-300 bg-white rounded-lg shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm {{ $class }}">
        {{ $slot }}
    </select>
</div>
